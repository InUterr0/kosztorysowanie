...
class Aplikacja(tk.Tk):
    ...
    def utworz_widgety(self):
        ...
        przycisk_generuj_dokument = ttk.Button(ramka_przyciski_koszyk, text="Generuj dokument", command=self.generuj_dokument)
        przycisk_generuj_dokument.pack(side=tk.LEFT, padx=5)
    ...
    def pobierz_zaznaczona_kategorie(self):
        zaznaczony_element = self.drzewo_kategorie.selection()
        if zaznaczony_element:
            nazwa_kategorii = self.drzewo_kategorie.item(zaznaczony_element[0], "text")
            kategorie = odczytaj_dane("kategorie.json")
            for kategoria in kategorie:
                if kategoria.nazwa == nazwa_kategorii:
                    return kategoria
...
def odswierz_podsumowanie(self, *args):
    ...
    # Obliczanie calkowitej marzy za projekt
...
def usun_kategorie(self):
    zaznaczona_kategoria = self.pobierz_zaznaczona_kategorie()
    if zaznaczona_kategoria:
        confirmation = messagebox.askyesno("Potwierdzenie", f"Czy na pewno chcesz usunąć kategorię {zaznaczona_kategoria.nazwa}?")
        if confirmation:
...
def generuj_dokument(self):
    robocizna = sum(p.cena * i for p, i in pobierz_zawartosc_koszyka() if p.typ == "robocizna")
    ilosc_dni_roboczych = self.ilosc_dni_roboczych
    material_bez_vat = sum(p.cena * i * 0.8 for p, i in pobierz_zawartosc_koszyka() if p.typ == "material")
    koszty_dojazdu = self.koszt_dojazdu if hasattr(self, 'koszt_dojazdu') else 0
    podwykonawcy = sum(p.cena * i for p, i in pobierz_zawartosc_koszyka() if p.typ == "podwykonawca")
    
    produkty_robocizna = [f"{p.nazwa} - {i}" for p, i in pobierz_zawartosc_koszyka() if p.typ == "robocizna"]
    produkty_material = [f"{p.nazwa} - {i}" for p, i in pobierz_zawartosc_koszyka() if p.typ == "material"]
    produkty_podwykonawcy = [f"{p.nazwa} - {i}" for p, i in pobierz_zawartosc_koszyka() if p.typ == "podwykonawca"]
    
    with open("kosztorys.txt", "w") as file:
        file.write(f"Robocizna: {robocizna:.2f}\n")
        file.write(f"Ilość dni roboczych: {ilosc_dni_roboczych:.2f}\n")
        file.write(f"Materiał bez VAT: {material_bez_vat:.2f}\n")
        file.write(f"Koszty dojazdu: {koszty_dojazdu:.2f}\n")
        file.write(f"Podwykonawcy: {podwykonawcy:.2f}\n\n")
        
        file.write("Produkty - Robocizna:\n")
        for produkt in produkty_robocizna:
            file.write(f"- {produkt}\n")
        
        file.write("\nProdukty - Materiał:\n")
        for produkt in produkty_material:
            file.write(f"- {produkt}\n")
        
        file.write("\nProdukty - Podwykonawcy:\n")
        for produkt in produkty_podwykonawcy:
            file.write(f"- {produkt}\n")
    
    messagebox.showinfo("Informacja", "Dokument został wygenerowany.")
