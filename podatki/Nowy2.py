...
def dodaj_koszt_odejmowany(nazwa_kosztu, kwota_kosztu, listbox_koszty_odejmowane):
    koszt = f"{nazwa_kosztu}: {kwota_kosztu:.2f} sek"
    listbox_koszty_odejmowane.insert(tk.END, koszt)
...
def oblicz_wynagrodzenie(kwota_faktury, stawka_godzinowa, koszty_projektu, koszty_bronek, koszty_mateusz, koszty_odejmowane,
    ...
    try:
        ...
        koszty_projektu_suma = sum([float(koszt.split(": ")[1].split(" ")[0]) for koszt in koszty_projektu])
        koszty_bronek_suma = sum([float(koszt.split(": ")[1].split(" ")[0]) for koszt in koszty_bronek])
        koszty_mateusz_suma = sum([float(koszt.split(": ")[1].split(" ")[0]) for koszt in koszty_mateusz])
        koszty_odejmowane_suma = sum([float(koszt.split(": ")[1].split(" ")[0]) for koszt in koszty_odejmowane])
        ...
        moms_do_zaplaty = moms_faktury - moms_kosztow_projektu - moms_kosztow_osobowych - (koszty_odejmowane_suma * 0.25)

        zysk_projektu = kwota_faktury - koszty_projektu_suma
        ilosc_godzin = (zysk_projektu - moms_faktury) / stawka_godzinowa
        ...
        zysk_firmowy = zysk_projektu - zysk_prywatny - moms_do_zaplaty - koszty_odejmowane_suma
...
def create_gui(window):
    ...
    frame_koszty_odejmowane = tk.Frame(window)
    frame_koszty_odejmowane.pack(pady=10)
    label_koszty_odejmowane = tk.Label(frame_koszty_odejmowane, text="Koszty odejmowane od zysku firmy:")
    label_koszty_odejmowane.pack()
    listbox_koszty_odejmowane = tk.Listbox(frame_koszty_odejmowane, width=50)
    listbox_koszty_odejmowane.pack(side=tk.LEFT)
    button_usun_koszt_odejmowany = tk.Button(frame_koszty_odejmowane, text="Usun koszt odejmowany",
                                             command=lambda: [usun_koszt(listbox_koszty_odejmowane), update_wynagrodzenie()])
    button_usun_koszt_odejmowany.pack(side=tk.LEFT, padx=10)
    ...
    frame_dodaj_koszt_odejmowany = tk.Frame(window)
    frame_dodaj_koszt_odejmowany.pack(pady=5)
    label_nazwa_kosztu_odejmowanego = tk.Label(frame_dodaj_koszt_odejmowany, text="Nazwa kosztu odejmowanego:")
    label_nazwa_kosztu_odejmowanego.pack(side=tk.LEFT)
    entry_nazwa_kosztu_odejmowanego = tk.Entry(frame_dodaj_koszt_odejmowany)
    entry_nazwa_kosztu_odejmowanego.pack(side=tk.LEFT)
    label_kwota_kosztu_odejmowanego = tk.Label(frame_dodaj_koszt_odejmowany, text="Kwota kosztu odejmowanego (sek):")
    label_kwota_kosztu_odejmowanego.pack(side=tk.LEFT, padx=10)
    entry_kwota_kosztu_odejmowanego = tk.Entry(frame_dodaj_koszt_odejmowany)
    entry_kwota_kosztu_odejmowanego.pack(side=tk.LEFT)
    button_dodaj_koszt_odejmowany = tk.Button(frame_dodaj_koszt_odejmowany, text="Dodaj koszt odejmowany",
                                              command=lambda: [dodaj_koszt_odejmowany(entry_nazwa_kosztu_odejmowanego.get(),
                                                                                   float(entry_kwota_kosztu_odejmowanego.get()),
                                                                                   listbox_koszty_odejmowane),
                                                              update_wynagrodzenie()])
    button_dodaj_koszt_odejmowany.pack(side=tk.LEFT, padx=10)
    ...
    def update_wynagrodzenie(event=None):
        koszty_projektu = [listbox_koszty.get(i) for i in range(listbox_koszty.size())]
        koszty_bronek = [listbox_koszty_bronek.get(i) for i in range(listbox_koszty_bronek.size())]
        koszty_mateusz = [listbox_koszty_mateusz.get(i) for i in range(listbox_koszty_mateusz.size())]
        koszty_odejmowane = [listbox_koszty_odejmowane.get(i) for i in range(listbox_koszty_odejmowane.size())]

        oblicz_wynagrodzenie(entry_faktura.get(), entry_stawka_godzinowa.get(),
                             koszty_projektu, koszty_bronek, koszty_mateusz, koszty_odejmowane,
                             label_wynik_projekt, label_wynik_bronek, label_wynik_mateusz)
